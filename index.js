...
async function run() {
    try {
        console.log("MongoDB setup ready.");
    } catch (err) {
        console.error(err);
    }
}
...
app.patch("/habits/:id", upload.single("image"), async (req, res) => {
    const { id } = req.params;
    const updatedData = req.body;
    if (req.file) updatedData.imageUrl = `http://localhost:${port}/uploads/${req.file.filename}`;
...
app.delete("/habits/:id", async (req, res) => {
    try {
        const result = await dbColl.deleteOne({ _id: new ObjectId(req.params.id) });
        if (result.deletedCount === 0) return res.status(404).json({ message: "Habit not found" });
        res.status(200).json({ message: "Habit deleted successfully" });
    } catch (err) {
        res.status(500).json({ message: "Failed to delete habit", error: err.message });
...
app.get('/myhabits/:email', async (req, res) => {
    try {
        const habits = await dbColl.find({ userEmail: req.params.email }).sort({ createdAt: -1 }).toArray();
        res.status(200).json(habits);
    } catch (err) {
        res.status(500).json({ message: "Failed to fetch user's habits", error: err.message });
    }
